...
public class RimozioneProfiliPerCDSUOApprovvigionamentiIT {

	private static final Logger log = LoggerFactory.getLogger(RimozioneProfiliPerCDSUOApprovvigionamentiIT.class);
	...
	public void runBatch() throws IOException {
		...
		for (int i = 0; i < persone.size(); i++) {
			rimuoviRuolo(persone.get(i).getPersona(), persone.get(i).getRuolo(), persone.get(i).getCdsuo(),persone.get(i).getData());
	...
	private void rimuoviRuolo(String username, String siglaRuolo, String NR_CDSUO, String data) {
					...
					for (int j = 0; j < listaRuoliUtente.size(); j++) {
						if (listaRuoliUtente.get(j).getEntitaOrganizzativa() != null) {
							...
							if (!utenteRuoloEoPresente) {
								System.out.println("UTENTE " + username + " non è associato al ruolo " + idRuolo + " - idPersona " + idPersona + " - idSede " + idSede);
							} else {
								System.out.println("Rimuovo ruolo " + idRuolo + " - idPersona " + idPersona + " - idSede " + idSede);

								try {
									//aceService.associaRuoloPersona(idRuolo, idPersona, idSede, java.time.LocalDate.parse(data),null,false,false,"","");
									aceBridgeService.personaEntitaOrganizzativaById(idSede).setFineValidita(null);;
	...
	private Map<Integer, associazioneRuoloPersonaCDSUO> getPersoneDaFile() throws IOException {
		...
		Stream<String> lines = Files.lines(Paths.get("./src/test/resources/batch/Approvvigionamenti-IT-Rimozione Utenti.csv"));
