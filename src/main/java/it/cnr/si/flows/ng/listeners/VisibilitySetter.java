...
import static it.cnr.si.security.PermissionEvaluatorImpl.*;
...
public class VisibilitySetter implements ActivitiEventListener {
    ...
    public void onEvent(ActivitiEvent event) {
        if (event.getType().equals(ActivitiEventType.TASK_CREATED)) {
            ...
            String idStruttureSupervisione = (String) variables.get(ID_STRUTTURE_SUPERVISIONE);
            if (idStruttureSupervisione!= null && !"".equals(idStruttureSupervisione)) {
                String[] idStruttureArray = idStruttureSupervisione.split(",");

                for (String idStrutturaSupervisione : idStruttureArray) {
                    runtimeService.addGroupIdentityLink(processInstanceId, "supervisore-struttura@"+ idStrutturaSupervisione , Utils.PROCESS_VISUALIZER);
                    if (processDefinitionKey != null && idStruttura!= null) {
                        runtimeService.addGroupIdentityLink(processInstanceId, "supervisore#"+ processDefinitionKey +"@"+ idStrutturaSupervisione, Utils.PROCESS_VISUALIZER);
                    }
                }
            }
        }
