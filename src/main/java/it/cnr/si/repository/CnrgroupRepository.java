...
public interface CnrgroupRepository extends JpaRepository<Cnrgroup,Long> {
    ...
    @Query("select cnrgroup from Cnrgroup cnrgroup left join fetch cnrgroup.parents where cnrgroup.name =:name")
    public Cnrgroup findOneWithEagerRelationshipsByName(@Param("name") String name);
