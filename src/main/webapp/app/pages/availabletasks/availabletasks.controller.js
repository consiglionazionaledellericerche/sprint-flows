(function () {
	...
	function AvailableTasksController($scope, paginationConstants, dataService, utils, $log) {
		...
		$scope.loadMyTasks = function () {
			...
			dataService.tasks.myTasks($scope.current, myFirstResult, myMaxResults, vm.order, utils.populateTaskParams(vm.searchParams))
		...
		$scope.loadAvailableTasks = function () {
			...
			dataService.tasks.myTasksAvailable($scope.current, firstResultAvailable, maxResultsAvailable, vm.order, utils.populateTaskParams(vm.searchParams))
				.then(function (response) {
					utils.refactoringVariables(response.data.data);
		...
		$scope.loadTaskAssignedInMyGroups = function () {
			...
			dataService.tasks.taskAssignedInMyGroups($scope.current, firstResultTAIMG, maxResultsTAIMG, vm.order, utils.populateTaskParams(vm.searchParams))
				.then(function (response) {
					utils.refactoringVariables(response.data.data);
